...
double get_audio_clock(VideoState *is) {
    ...
    if(bytes_per_sec) {
        pts -= (double)hw_buf_size / bytes_per_sec; // audio 缓冲区剩余的数据需要播放时间， pts 减去这部分时间就是当前audio的pts
    }
    return pts;
}
    ...
    for(;;) {
        while(is->audio_pkt_size > 0)
        {
            ...
            *pts_ptr = pts;
            ...
            // 根据Sample rate 和 sample format 来计算出该packet中的数据可以播放的时长
            // 此时的 audio_clock packet有过个音频，可能解码出来后没来得及播放，所以真实的音频pts可能小于此时的 audio_clock , 在get_audio_clock () 函数中进行了矫正
            is->audio_clock += (double)t_audio_size /(double)(n * is->audio_st->codec->sample_rate);
            
//            is->audio_clock += (double)data_size /
//            (double)(n * is->audio_st->codec->sample_rate);
            
            /* We have data, return it and come back for more later */
            /*一旦解码出来数据就直接return了，没有判断is->audio_pkt_size是否还有数据没有解码，这里，调用一次解码一次*/
            //return data_size;
            return t_audio_size;
        }
        if(pkt->data)
        ...
        if(pkt->pts != AV_NOPTS_VALUE) {
            // 设置audio_clock为该packet的pts
            // 一个音频packet中可能包含多个音频帧，所以packet中的pts比真正音频播放的pts可能早很多
...
void video_refresh_timer(void *userdata) {
    ...
    if(is->video_st) {
        if(is->pictq_size == 0) {
            ...
            diff = vp->pts - ref_clock; // diff 来标示 视频的播放速度是快了还是慢了
            
            /* Skip or repeat the frame. Take delay into account
             FFPlay still doesn't "know if this is the best guess." */
            sync_threshold = (delay > AV_SYNC_THRESHOLD) ? delay : AV_SYNC_THRESHOLD;
            if(fabs(diff) < AV_NOSYNC_THRESHOLD) {
                if(diff <= -sync_threshold) {   // diff < 0 视频播放慢了
                    delay = 0;          /*视频播放慢了*/
                } else if(diff >= sync_threshold) { // diff > 0 , 视频播放快了
                    delay = 2 * delay;  /*视频播放快了则加倍延时*/
                }
            }
            ...
            /*
            1.使用当前帧的pts减去上一帧的pts来估计播放下一帧的延迟时间，delay
            2.根据视频时钟和音频时钟的时间差diff来判断 video播放的快慢
            3.并根据video的播放速度来调整这个延时时间，来实现音视频的同步播放
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {
    ...
    if(!vp->m_pFrame ||
       ...
       vp->height != is->video_st->codec->height) {
        ...
        event.type = FF_ALLOC_EVENT;  //  alloc to do it in main thread.
        event.user.data1 = is;
        SDL_PushEvent(&event);
...
int video_thread(void *arg) {
    ...
    for(;;) {
        ...
        av_free_packet(packet); // 从queue中取出的packet 需要及时的释放
    }
    av_free(pFrame);
    return 0;
}
...
//    const char * fileName = "/zjcletv/Desktop/FFmpeg-Learning/FFmpeg-tutorial/40M.mp4";
    ...
    // rtmp://ossrs.net:1935/live/demo?vhost=players.1496828019977
    const char * fileName = "rtmp://ossrs.net:1935/live/demo?vhost=players.1496828019977";
    ...
    for(;;) {
        ...
        switch(event.type) {
            ...
            case FF_REFRESH_EVENT:
                video_refresh_timer(event.user.data1); // 决定什么时候渲染 视频
                break;
            default:
                break;
        }
