...
int video_thread(void *arg) {
    ...
    for(;;) {
        ...
        if(packet->data == flush_pkt.data) {    // 收到清空包时， 把avcode内部的缓冲清空
            // avcode 有自己的内部缓冲， 也需要每次被清空
    ...
    for(;;) {
        ...
        if(is->seek_req) {
            ...
            if(av_seek_frame(is->pFormatCtx, stream_index, seek_target, is->seek_flags) < 0) {
                ...
                /*
                 1. 首先写一个函数来清空我们的包队列。
                 2. 然后我们需要一种通知音频和视频线程来清空 avcodec 内部缓冲的办法。我们可以在清空队列后把特定的包(flush_pkt)放入到队列中，
                 3. 然后当音频和视频线程检测到flush_pkt包的时候，avcodec_flush_buffers 清空解码时缓存的数据
        ...
        if(packet->stream_index == is->videoStream) {
            /*
             1.在调用packet_queue_put时传递的是指针，也就是形参pkt和实参packet中的data引用的是同一个数据缓存。
             2.但是在循环调用av_read_frame的时候，会将packet中的data释放掉，以便于读取下一个帧数据
             3.
             */
